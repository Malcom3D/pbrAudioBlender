...
from .particles_exporter import ParticlesExporter
...
#        obj.to_mesh_clear()
        ...
        if not quest_mesh.is_volume and self.to_add(name):
            ...
            if not quest_mesh.is_volume:
                ...
                print(f"{obj.name_full} is empty mesh:", quest_mesh.is_empty)
                print(f"{obj.name_full} is faces convex:", quest_mesh.is_convex)
                print(f"{obj.name_full} is winding consistent:", quest_mesh.is_winding_consistent)
                print(f"{obj.name_full} is mesh watertight:", quest_mesh.is_watertight)
                print(f"{obj.name_full} is mesh volumetric:", quest_mesh.is_volume)
...
#        print(f"to_add: {self.to_add(name)} object: {name} not_valid: {self.not_valid}")
    ...
    def export_particle_systems(self, obj: bpy.types.Object, start_frame: int = None, end_frame: int = None):
        ...
        particles_exporter = ParticlesExporter(scene=scene, decimals=self.decimals)
        
        # Export each particle system
        for psys in obj.particle_systems:
            # Use per-material splitting for collection rendering
            particle_config = particles_exporter.export_particle_system_by_material(
