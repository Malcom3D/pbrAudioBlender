...
class PBRAudioRenderEngine(RenderEngine):
    ...
    def _post_process_results(self, output_dir, scene):
        ...
        # Check if graphical preview is enabled
        if scene.pbraudio.enable_graphical_preview:
            ...
            try:
                ...
                if os.path.exists(ray_data_dir):
                    # Execute rays visualization directly
                    self._visualize_rays(ray_data_dir)
                    self.report({'INFO'}, "Ray visualization completed")
                else:
    ...
    def _visualize_rays(self, ray_data_dir):
        ...
        # Create collection for rays
        collection_name = bpy.context.scene.pbraudio.viewRays_collection
        ...
        if collection_name not in collections:
            ...
            # Clear existing rays if collection already exists
        ...
        # Find all JSON files
        json_files = glob.glob(os.path.join(ray_data_dir, "*.json"))
        
        if not json_files:
            self.report({'WARNING'}, "No ray data JSON files found")
            return
        ...
        for json_file in sorted(json_files):
            try:
                ...
                # Create rays for this file
                self._create_rays_mesh(origins, hit_points, collection)
                
                self.report({'INFO'}, f"Processed {os.path.basename(json_file)}: {len(origins)} rays")
                
            except Exception as e:
                self.report({'WARNING'}, f"Error processing {json_file}: {str(e)}")
                continue
        
        self.report({'INFO'}, f"Total rays visualized from {len(json_files)} files")

    def _create_rays_mesh(self, origins, hit_points, collection):
        """Create efficient ray visualization using a single mesh"""
        # Create vertices and edges
        vertices = []
        edges = []
        
        for origin, hit_point in zip(origins, hit_points):
            start_idx = len(vertices)
            vertices.append(tuple(origin))
            vertices.append(tuple(hit_point))
